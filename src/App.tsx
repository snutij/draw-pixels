...
const CANVAS_SIZE = 500,
  ...
  App = (): ReactElement => {
    const [dimension, setDimension] = useState<number>(DEFAULT_DIMENSION),
      [random, setRandom] = useState<boolean>(false),
      [erase, setErase] = useState<boolean>(false),
      handleDimensionChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
      ...
      applyModeSwitch = (key: string): void => {
        if (key === "e") {
          setRandom(false);
          setErase((prev) => !prev);
        } else if (key === "r") {
